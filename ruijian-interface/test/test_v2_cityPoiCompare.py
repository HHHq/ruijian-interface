...
excel_data = ExcelHandler(config.excel_path).read_sheet("睿见V2_城市甄选_按POI排序")
request_url = read_yaml(config.yaml_path)["env_ruijian"]["url"]
logger = MidWare.logger


@ddt.ddt
class TestV2cityPoiCompare(unittest.TestCase):
    def setUp(self):
        self.cookie = MidWare().get_cookies()

    @ddt.data(*excel_data)
    def test_v2_citypoicompare(self, info):
        logger.info("开始请求--->按poi排序接口-用例id_{}".format(info["case_id"]))
        ...
        try:
            ...
            logger.info("城市筛选_按poi排序接口_测试用例id_{}通过".format(info["case_id"]))
        except Exception as e:
            logger.error("城市筛选_按poi排序接口_测试用例id_{}不通过".format(info["case_id"]))
            raise e
