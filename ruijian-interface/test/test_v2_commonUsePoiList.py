...
excel_data = ExcelHandler(config.excel_path).read_sheet("睿见V2_常用poi点列表")
request_url = read_yaml(config.yaml_path)["env_ruijian"]["url"]
logger = MidWare.logger


@ddt.ddt
class TestV2CommonUsePoiList(unittest.TestCase):
    def setUp(self):
        self.cookie = MidWare().get_cookies()

    @ddt.data(*excel_data)
    def test_v2_CommonUsePoiList(self, info):
        logger.info("开始请求--->常用poi点列表接口-用例id_{}".format(info["case_id"]))
