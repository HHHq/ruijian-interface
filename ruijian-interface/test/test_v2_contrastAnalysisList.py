...
excel_data = ExcelHandler(config.excel_path).read_sheet("睿见V2_对比分析")
request_url = read_yaml(config.yaml_path)["env_ruijian"]["url"]
logger = MidWare.logger


@ddt.ddt
class TestV2ContrastAnalysisList(unittest.TestCase):
    def setUp(self):
        self.cookie = MidWare().get_cookies()

    @ddt.data(*excel_data)
    def test_v2_citysorttree(self, info):
        logger.info("开始请求--->对比分析接口-用例id_{}".format(info["case_id"]))
        ...
        try:
            ...
            logger.info("对比分析接口_测试用例id_{}通过".format(info["case_id"]))
        except Exception as e:
            logger.error("对比分析接口_测试用例id_{}不通过".format(info["case_id"]))
            raise e
